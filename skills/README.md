...
The triage output categorizes PRs into recommendations based on each repo's `relationship` in `policies/repo-allowlist.yaml`: **maintained** repos get review actions (`deep-review`, `scan-review`, `monitor`), while **dependency** repos get awareness-only actions (`assess-impact`, `watch`) — a dependency-repo PR never gets `deep-review`, even at critical severity. The actual review follow-through for maintained repos is performed by a human or by the separate [pr-review](pr-review/SKILL.md) skill. `scripts/macos/daily-command-center.sh` consumes the triage report and keeps the two categories in separate "Needs Attention" (maintained) and "Upstream Alerts" (dependency) tables.
